...
class UsageNow(Resource):
    def get(self):
        return {
            'cpu': f"{str(psutil.cpu_percent())}%",
            'temperature':  f"{str(CPUTemperature().temperature)}C",
            'memory':  f"{str(psutil.virtual_memory()[2])}%"
        }
...
#Class
class SensorValue:
    def __init__(self, temperature, humdity, location):
...
def read(threadName, runningFlag, readDelay, sensortype, pin1, pin2, sensorRetry, groupingSize):
    while runningFlag:
        ...
        # this block is pointless inside is always position [0]
        if(readings[0].location == 'inside'):
            ...
            last_read_datetime       = readings[2]
            last_temperature_inside  = readings[1].temperature
            last_humdity_inside      = readings[1].humdity

            last_temperature_outside = readings[0].temperature
            last_humdity_outside     = readings[0].humdity
...
def readSensors(sensor_type, sensor_pin_4_inside, sensor_pin_22_outside, bool_sensor_retry):
    ...
    if bool_sensor_retry == True:
        ...
        return (SensorValue(temperature1, humidity1,"inside"), SensorValue(temperature2, humidity2,"outside"), timestamp) 
    else:
